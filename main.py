...
def build_folder_menu(menu: QMenu, folder_path: str, depth=0, max_depth=3, show_files=True):
    ...
    for path in ordered:
        ...
        if os.path.isdir(path):
            ...
            open_here.triggered.connect(lambda checked=False, p=path: os.startfile(p))
            ...
            act.triggered.connect(lambda checked=False, p=path: os.startfile(p))
            menu.addAction(act)
...
class TrayApp(QObject):
    ...
    def __init__(self):
        ...
        # Two menus: folder_menu for left click, context_menu for right click
    ...
    def _populate_menus(self):
        ...
        for root in valid_roots:
            ...
            open_root.triggered.connect(lambda checked=False, p=root: os.startfile(p))
        ...
        edit_action.triggered.connect(lambda: os.startfile(CFG_PATH))
...
if __name__ == "__main__":
    # Create cfg file if missing, auto-detecting current user's main folders
